...
// Dan's top 3: Port of Newcastle (690059), Mt Crosby Pumping Station (630006), Burpengary Wastewater (1200018)
const projectIds = [690059, 630006, 1200018];
for (const pid of projectIds) {
  const [proj] = await conn.execute('SELECT id, name, location, stage FROM projects WHERE id = ?', [pid]);
  ...
  contacts.forEach((c, i) => {
    ...
    console.log(`  ${marker} ${c.name} | ${c.title} | ${c.contactTrustTier} | ${c.roleRelevance} | ${c.email || 'no-email'}`);
  });
}
await conn.end();
