...
type PolicyProject = ThisWeekProject & {
  application?: string;
  commercialTruth?: PortableAirCommercialTruth | PortableAirCommercialPolicyResult;
...
function failClosedForMissingPreferredBuyer(
  truth: PortableAirCommercialPolicyResult,
): PortableAirCommercialPolicyResult {
  if (!truth.actionReady) return truth;
  const hasHolder = truth.recordedPackageHolders.length > 0;
  return {
    ...truth,
    actionReady: false,
    preferredBuyerContactId: null,
    buyerStatus: hasHolder ? "find_buyer" : "map_package_holder",
    recommendedAction: hasHolder ? "find_contacts" : "map_package_holder",
    whyNow: "The route appears commercially relevant, but the package-matched safe buyer could not be re-established for this response.",
    bestNextMove: hasHolder
...
async function applyPolicyToProject(
  ...
  const preferred = matchedDossierContact(dossier, policyTruth.preferredBuyerContactId);
  const truth = policyTruth.actionReady && !preferred?.effectivelySendReady
    ? failClosedForMissingPreferredBuyer(policyTruth)
    : policyTruth;
  const finalPreferred = matchedDossierContact(dossier, truth.preferredBuyerContactId);

  const safeStakeholder = truth.actionReady && finalPreferred?.effectivelySendReady
    ? {
...
function policySuggestedAction(project: PolicyProject): SuggestedAction | null {
  const truth = project.commercialTruth as PortableAirCommercialPolicyResult | undefined;
...
export async function getThisWeekSummary(
  ...
  const commerciallyGated = await Promise.all(
    summary.topProjects.map(project => applyPolicyToProject(project as PolicyProject, repName)),
  );
