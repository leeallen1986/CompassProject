#!/usr/bin/env tsx
/**
 * Read-only Rental Hire coverage census.
 *
 * Usage:
 *   pnpm exec tsx server/scripts/fullPotentialRentalCoverageCensus.ts \
 *     --output-dir artifacts/rental-coverage/<utc-run>
 *
 * Optional external research candidate file:
 *   --candidate-file path/to/rental-candidates.csv
 *
 * The command performs database SELECTs only. It has no apply/commit mode and
 * does not call any provider, LLM, discovery, pipeline or email service.
 */
...
import {
  ...
  type RentalCoverageCensusRow,
  type RentalCoverageCandidateResult,
...
export function parseRentalCoverageArgs(argv: string[]): RentalCoverageCliOptions {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (["--apply", "--commit", "--seal", "--write-db", "--migrate"].includes(arg)) {
      throw new Error(`${arg} is not supported; the Rental Hire coverage census is read-only.`);
    } else {
...
function candidateTemplateCsv(): string {
  const example = {
    ...
    notes: "Do not import until canonical and route review is complete",
    excludeReason: "",
  };
  return toCsv([example], CANDIDATE_TEMPLATE_HEADERS);
}
...
const CENSUS_HEADERS: Array<keyof RentalCoverageCensusRow> = [
  "accountId",
  "stableKey",
  "canonicalName",
  "displayName",
...
const CANDIDATE_RESULT_HEADERS: Array<keyof RentalCoverageCandidateResult> = [
  "candidateName",
  "parentName",
  "website",
  "state",
...
export async function runRentalCoverageCensusCli(argv = process.argv.slice(2)): Promise<void> {
  ...
  const writes: Array<Promise<void>> = [
    ...
    writeFile(path.join(outputDir, "rental-coverage-gap-queue.csv"), toCsv(result.gapQueue as unknown as Record<string, unknown>[], CENSUS_HEADERS), "utf8"),
  ...
  if (options.candidateFile) {
    writes.push(
      writeFile(
        path.join(outputDir, "rental-coverage-candidate-reconciliation.csv"),
        toCsv(result.candidateReconciliation as unknown as Record<string, unknown>[], CANDIDATE_RESULT_HEADERS),
        "utf8",
      ),
      writeFile(
        path.join(outputDir, "rental-coverage-candidate-reconciliation.json"),
        `${JSON.stringify(result.candidateReconciliation, null, 2)}\n`,
        "utf8",
      ),
    );
  }
  ...
  process.stdout.write(`${JSON.stringify({
    mode: "read_only_rental_coverage_census",
    outputDir,
