...
function rowsToCsv(rows: RyanPortfolioAuditRow[]): string {
  const header = [
    ...
    "productFitProven",
    "packageHolderCount",
    "recordedPackageHolderCount",
    "exactContactCount",
    "namedContactCount",
    "effectiveSendReadyCount",
    "buyerLaneContactCount",
    "effectiveBuyerContactCount",
    "packageMatchedBuyerContactCount",
    "effectivePackageMatchedBuyerCount",
  ...
  const body = rows.map(row => [
    ...
    row.metrics.recordedPackageHolderCount,
    ...
    row.metrics.packageMatchedBuyerContactCount,
    row.metrics.effectivePackageMatchedBuyerCount,
    row.metrics.principalOrReferralContactCount,
    row.metrics.cardMatchesPackageBuyer,
...
function reportToMarkdown(report: RyanPortfolioAuditReport): string {
  ...
  const worstRows = report.worst15.length > 0
    ? report.worst15.map((row, index) => [
        ...
        `- Package holders (all / recorded): ${row.metrics.packageHolderCount} / ${row.metrics.recordedPackageHolderCount}`,
        `- Exact contacts: ${row.metrics.exactContactCount}`,
        `- Effective buyer-lane contacts: ${row.metrics.effectiveBuyerContactCount}`,
        `- Package-matched buyer contacts: ${row.metrics.packageMatchedBuyerContactCount}`,
        `- Effective package-matched buyers: ${row.metrics.effectivePackageMatchedBuyerCount}`,
        `- Card matches package buyer: ${row.metrics.cardMatchesPackageBuyer}`,
