...
function qualificationUniverseTable(view: FullPotentialSeptemberManagementView): string[] {
  ...
  const classSummary = view.qualificationUniverse.byModelBand
    ...
    .join("; ");
  const ts2Positions = view.qualificationUniverse.ts2SurfacePositionUniverse;
  const ts3Positions = view.qualificationUniverse.ts3UndergroundPositionUniverse;
  ...
  return [
    ...
    `**TS2 provisional three-year position universe (Low / Base / High):** ${ts2Positions.low} / **${ts2Positions.base}** / ${ts2Positions.high}  `,
    `**TS3 provisional three-year position universe (Low / Base / High):** ${ts3Positions.low} / **${ts3Positions.base}** / ${ts3Positions.high}`,
    "",
    "> TS2 and TS3 position universes are non-monetary qualification sensitivities. They are not installed equipment, pipeline, customer intent or sales forecasts. No value enters Full Potential until a distinct buyer application is proven and separately reviewed.",
    "",
